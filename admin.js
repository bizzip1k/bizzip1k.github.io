...
function postSortTime(p){if(p&&p.publishedAt){const t=Date.parse(p.publishedAt);if(!Number.isNaN(t))return t}if(p&&p.date){const t=Date.parse(String(p.date)+"T00:00:00");if(!Number.isNaN(t))return t}return 0}
function sortPostsNewestFirst(a,b){return postSortTime(b)-postSortTime(a)}

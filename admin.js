...
/* CONTENTS */
async function loadContents(){const [pg,lg]=await Promise.all([get(CFG.posts),get("contents.html")]);S.contents.sha=pg.sha;S.contents.posts=JSON.parse(d64(pg.content));S.contents.landingSha=lg.sha;S.contents.landingHtml=d64(lg.content);const d=doc(S.contents.landingHtml);$("ctLandingTitle").value=text(d,".page-hero h1");$("ctLandingSummary").value=text(d,".page-hero h1 + p");renderPostList();fillPostCategories();newPost()}
function renderPostList(){$("postList").innerHTML=S.contents.posts.slice().sort((a,b)=>String(b.date).localeCompare(String(a.date))).map(p=>{const i=S.contents.posts.findIndex(x=>x.id===p.id);return `<div class="list-row"><div><strong>${esc(p.title)}</strong><small>${esc(p.date||"")} · ${esc(p.categoryLabel||"미분류")}${p.subcategoryLabel?" / "+esc(p.subcategoryLabel):""}</small></div><button class="btn light mini ep" data-i="${i}">수정</button></div>`}).join("");document.querySelectorAll(".ep").forEach(b=>b.onclick=()=>editPost(+b.dataset.i))}
...
function newPost(){S.contents.edit=-1;$("postTitle").value="";$("postDate").value=today();$("postSummary").value="";S.contents.editSections=[{heading:"새 소제목",paragraphs:[""],bullets:[]}];renderPostSections();$("savePost").disabled=false;previewPost()}
...
function previewResourceLanding(){
  ...
  <div class="pv-section"><div class="pv-grid">${S.resources.cards.map(x=>`<div class="pv-item"><strong>${esc(x.title)}</strong><small>${esc(x.summary)}</small></div>`).join("")}</div></div>`;
}
